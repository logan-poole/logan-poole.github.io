...
(function () {
  ...
  function injectIntoMenu(menu) {
    ...
    hr.style.borderTop = "1px solid var(--border, rgba(0,0,0,.12))";
  ...
  // Only add a fallback top-nav "Admin" link if no profile menu exists
  function injectFallbackNav() {
    ...
    a.textContent = "Admin";
    // place before the theme toggle for consistency
  ...
  document.addEventListener("DOMContentLoaded", once(async () => {
    ...
    try {
      ...
      console.warn("[admin-profile-menu] cannot determine admin status:", e);
    }
  }));
})();
